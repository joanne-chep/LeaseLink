...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image_descriptions = $_POST['image_descriptions'] ?? []; // Array of descriptions

    // Validate required fields (excluding main_image_url now, as it's handled by $_FILES)
    ...
    if (!empty($_FILES['property_images']['name'][0])) {
        foreach ($_FILES['property_images']['name'] as $key => $name) {
            ...
            if (move_uploaded_file($file_tmp, $destination)) {
                $uploaded_image_paths[] = [
                    'url' => 'assets/property_images/' . $new_file_name, // Store relative path
    ...
    if ($stmt_property->execute()) {
        ...
        // Insert all uploaded images into property_images table
        foreach ($uploaded_image_paths as $key => $image_data) {
            $is_main = ($key === 0) ? TRUE : FALSE; // First image is main
            ...
            if ($stmt_image === false) {
                ...
                continue; // Continue to next image if one fails
            }
            $stmt_image->bind_param("isss", $property_id, $image_data['url'], $image_data['description'], $is_main);
        ...
        // If property insertion fails, clean up uploaded files
